...
│   └── lib/
│       ├── learn-helpers.mjs    # Pure helpers for learn-catalog-sync (unit-tested)
│       └── docs-helpers.mjs     # Pure helpers for docs-catalog-sync (unit-tested)
├── test/
│   ├── learn-helpers.test.mjs   # node:test unit tests (npm test)
│   └── docs-helpers.test.mjs
├── data/
...
└── .github/
    ...
    └── workflows/
        ├── ci.yml                       # Syntax + unit tests + actionlint on push/PR (not a scheduled sync)
...
- **Workflow:** `.github/workflows/learn-catalog-monitor.yml` — runs Monday 07:00 UTC + manual trigger; commits the refreshed file directly (no PR) and opens/updates an issue only if the sync fails.
...
### No-op write skip (added 2026-09-04)
The output's `lastChecked` timestamp is always fresh, which used to guarantee a data commit every single week even when nothing changed. The script now compares the new output against the existing file *ignoring* `lastChecked` and skips the write entirely when only the timestamp would differ — so the weekly workflow's "commit if changed" step genuinely no-ops on unchanged weeks.
...
- **Script:** `scripts/docs-catalog-sync.mjs` — Node (ESM, zero dependencies; `git` only for the github/docs source). Run with `node scripts/docs-catalog-sync.mjs` (`DRY_RUN=1` to preview).
- **Workflow:** `.github/workflows/docs-catalog-monitor.yml` — runs Monday 08:00 UTC + manual trigger; commits the refreshed file directly (no PR, same pattern as the Learn Catalog workflow) and opens/updates an issue if any repo fails or the sync falls below the failsafe threshold. The commit step runs even if the sync step reports failure (`if: ${{ !cancelled() }}`), so a partial failure (one repo down) still commits the data gathered from the repos that succeeded — and since 2026-09-04 the failed repo's previous entries are carried forward into that commit too (see Automated link checking below), so a one-week outage doesn't punch a hole in the catalog.
- **Source (since 2026-09):** Learn's own sitemaps, not `MicrosoftDocs/*` repos. Microsoft Learn announced on 2026-09-23 that it is retiring most public documentation repos by the end of December 2026, and a retired repo is no longer publicly visible at all (clones fail). The git-based design described in older sections below is historical.

### How the sitemap source works
1. **Discovery:** `learn.microsoft.com/_sitemaps/sitemapindex.xml` lists child sitemaps named `<family>_<locale>_<n>.xml`. Only `en-us` files are read. Every `<loc>` is normalized to the catalog's existing URL shape (strip `/en-us`, strip `?view=` moniker, strip trailing slash) and kept if it's under a `LEARN_SCOPE.include` prefix and not under an `exclude` prefix. Scope is prefix-based now, so it's a superset of the old repo-based coverage (e.g. all of `microsoft-365/`, including admin docs that never had a public repo).
2. **Family memo:** `data/docs-sitemap-families.json` records which sitemap families contained in-scope URLs. Weekly runs skip families marked `relevant: false`; unknown (new) families are always scanned. Run `FULL_DISCOVERY=1` occasionally to re-check skipped families.
3. **Change detection:** each record now carries `lastmod` (the sitemap's per-URL lastmod). A page is fetched only if it's new or its lastmod changed. Records without a `lastmod` field (built by the old git script) adopt the sitemap value on first sight, so the migration costs ~0 page fetches.
4. **Metadata:** only the `<head>` of each queued page is read: `<title>` (cleaned of ` | Microsoft Learn`), `meta description`, `ms.service`, `ms.subservice`. Pages with a robots `noindex` are skipped. Capped by `MAX_PAGE_FETCHES` (default 6000); over-cap *changed* pages keep their old record and old lastmod, so they re-queue next run.
5. **Removals:** a cataloged Learn URL missing from every sitemap is fetched: 404/410 → quarantined (same model as before); redirect to a different page → dropped as moved; 200 → kept. If any sitemap file failed to download, removal detection is skipped for that run and missing entries are carried forward.
6. **Safety:** the run aborts without writing if the sitemaps yield < 50% of last run's Learn URL count (sitemap outage/format change).

**Title note:** `<title>` includes docfx's `titleSuffix` (e.g. "Citus Overview - Citus for PostgreSQL"), whereas the old frontmatter-based titles usually didn't. Records refreshed via the sitemap source will gradually pick up the suffixed form.

**docs.github.com** still comes from a `git clone` of `github/docs` (`GIT_SOURCES`). It's GitHub's own open-source repo, not a Microsoft Learn documentation repo, so the retirement doesn't apply.
...
- ✅ **SupportArticles-docs (done, added 2026-08-31; Office-family folders added 2026-09-04):** the entire `learn.microsoft.com/troubleshoot/...` URL namespace across many products is **not** part of `azure-docs` or any other already-integrated repo — it's a dedicated repo, `MicrosoftDocs/SupportArticles-docs` (public sync of `SupportArticles-docs-pr`). Its top-level `support` folder is the source for `troubleshoot/...` (folder name `support` → URL segment `troubleshoot`, verified via two live pages' `original_content_git_url`/`source_path`, e.g. `support/azure/private-link/troubleshoot-private-endpoint-connectivity-problems.md` → `troubleshoot/azure/private-link/troubleshoot-private-endpoint-connectivity-problems`). **6,272 entries.** The repo's 7 other published docsets — the Office-family troubleshooting content whose *general admin* docs are confirmed closed (see above) — were added 2026-09-04: `Exchange`, `Microsoft365`, `Office`, `Outlook`, `SharePoint`, `Teams`, `Viva` (~1,450 entries). **Critical mapping gotcha:** for 5 of the 7 (Exchange/Microsoft365/Office/Outlook/SharePoint), each docset has *multiple src dirs all publishing to `dest: "."`* — the second-level folder name is STRIPPED from the live URL (e.g. `Exchange/ExchangeServer/mailflow/dns-query-failed.md` → `troubleshoot/exchange/mailflow/dns-query-failed`). This is expressed in `REPOS` by using each second-level dir as its own `sourceFolder` (18 targets on the one repo entry). Teams and Viva are plain one-folder docsets. Renamed URL segments to watch: `Microsoft365` → `troubleshoot/microsoft-365` (hyphenated) and `Teams` → `troubleshoot/microsoftteams`. `SkypeForBusiness` and `windows` top-level folders contain zero markdown and aren't in `docsets_to_publish` — skipped. Cross-src URL collisions within each flattened namespace were checked empirically (0 found); the built-in duplicate-URL audit guards against future ones.
- **Azure split-out repo gap — FIXED 2026-09-04:** Microsoft has been splitting content out of the monolithic `azure-docs` repo into topic-specific repos (confirmed via live pages' `original_content_git_url`, e.g. VM Scale Sets → `azure-compute-docs`, Azure portal → `azure-management-docs`). A dedicated enumeration pass found exactly **six** public split repos, all now in `REPOS`: `azure-compute-docs` (VMs/VMSS/container-instances/service-fabric, ~2,040 md), `azure-ai-docs` (ai-services/foundry/machine-learning/search, ~5,024 md — the largest; its top-level `agent-framework/` dir sits outside `articles/` and is not indexed, and `articles/machine-learning/v-fake` is a moniker artifact skipped via `SKIP_DIRS`), `azure-monitor-docs` (azure-monitor/advisor/chaos-studio/service-health, ~2,832 md), `azure-management-docs` (azure-arc/azure-portal/container-registry/copilot/lighthouse, ~864 md), `azure-aks-docs` (aks/kubernetes-fleet, ~898 md), and `azure-security-docs` (key-vault/attestation/HSMs/confidential-ledger, ~1,000 md — **archived on GitHub yet still receiving live syncs from its `-pr` counterpart and still clonable**; if its clone ever fails, look for a successor repo). All six use the identical `sourceFolder: "articles"` → `baseUrlPath: "azure"` mapping as `azure-docs` (same docfx `src: articles, dest: "."` pattern, verified against live pages). Probed-and-nonexistent splits: `azure-networking-docs`, `azure-storage-docs`, `azure-databases-docs`, `azure-iot-docs` — networking/storage/databases/IoT content still lives in `azure-docs`. Crucially, `azure-docs` retains **no stubs** for the migrated folders (they were deleted outright, redirects live server-side in the split repos' `.openpublishing.redirection.*.json`), so indexing all seven repos under the shared `azure/` namespace produces no duplicate URLs — and the sync script's built-in duplicate-URL audit (see below) would catch it if that ever changes.
...
- **Final total: ~88k entries expected after the next sync** (74,806 before the 2026-09-04 additions of the six Azure split repos (~12.7k) and the SupportArticles Office-family folders (~1.45k) — the exact number lands with the next scheduled run). Easy/Medium/Dynamics 365/AI Business/SupportArticles-docs/Azure-split tiers are done; Office-suite family general admin docs and Windows client/Agent 365 confirmed closed.
...
### Automated link checking (added 2026-08-31; quarantine model added 2026-09-01; hardened 2026-09-04)
...
- **Every new/changed URL** (present in this run's output but not in the previous `docs-catalog.json`) is checked, capped at `LINK_CHECK_MAX_NEW` (2,000) per run — this is the highest-value signal since it catches a wrong `baseUrlPath`/target mapping (like the Dynamics 365 bug) immediately, on the exact run that introduced it. When a big new repo lands more than the cap, a random 2,000-URL sample is checked (statistically plenty to detect a wrong mapping — a bad base path 404s en masse) and the spill-over enters the catalog unchecked, getting sampled on later runs.
- **A random sample of unchanged existing URLs** (`LINK_CHECK_SAMPLE_SIZE`, currently 1,500 per run) is also checked, to catch upstream drift (Microsoft renaming/moving/retiring a page) even when our own script didn't change. At this sample size the full catalog cycles roughly once a year across weekly runs — not immediate, but sustainable within `learn.microsoft.com`'s rate limits (see below).
- **Checking the whole catalog every run isn't feasible**: `learn.microsoft.com` returns HTTP 429 aggressively above ~3 concurrent requests (confirmed by trial — concurrency 12 made almost every request fail with 429; concurrency 3 with a 500ms per-worker delay and exponential backoff on 429 works reliably). A full pass at that safe rate would take 20+ hours.
- **Only DEFINITIVE failures (HTTP 404/410) quarantine a URL** (since 2026-09-04): the URL is pulled out of `docs-catalog.json` and appended to `data/docs-catalog-invalid.json` (same shape as a normal entry, plus `status`, `firstDetected`, `lastChecked`). Transient outcomes — timeouts, network errors, 5xx, persistent 429 — are logged and skipped: the entry stays in the catalog and gets re-sampled on a later run, so a rate-limit burst can't poison the quarantine file with false positives (the surge callout in the CI issue remains as a second line of defense for anything that slips through as a real 404 burst).
- **Quarantined URLs are re-checked every run and auto-released** (since 2026-09-04): the quarantine list is small (tens of records), so re-checking all of it is nearly free. A record that now resolves is deleted from `docs-catalog-invalid.json` automatically and its page re-enters the catalog on the same run; records that stay definitively broken get `lastChecked` updated (kept truthful). Manual removal of records is no longer needed for restored pages — only genuinely-retired pages accumulate, awaiting the periodic agent triage pass (find each page's replacement or root cause, fix it at the source, and delete the resolved record(s)).
- **A whole-catalog duplicate-URL audit runs every time** (since 2026-09-04): two entries claiming the same URL means a wrong `REPOS` mapping (the manual version of this check is how the Dynamics 365 bug was caught). Small counts are deduplicated (first occurrence wins) with a loud warning; more than `DUPLICATE_URL_FAIL_THRESHOLD` (50) aborts the run as a systemic mapping bug.
- **Failed repos carry forward** (since 2026-09-04): if a repo's clone/parse fails, the previous catalog's entries under that repo's URL prefixes (see `repoUrlPrefixes()` in `scripts/lib/docs-helpers.mjs`) are carried into this run's output instead of silently vanishing for a week — which would also have made them all count as "new/changed" on the next successful run and blown the link-check budget. The run still exits non-zero and opens/updates the CI failure issue.
- Only a genuine repo clone/parse failure (`failedRepos`) or the duplicate-threshold abort fails the run/opens a CI issue now — a growing quarantine list is an expected, self-managed steady state, not a build failure.
- The output file is **sorted by URL** before writing (since 2026-09-04), so diffs between refreshes are stable regardless of `REPOS` order or directory-walk order — same rationale as the Learn catalog's sort-by-uid.
...
- **`.github/workflows/ci.yml`** — runs on every push/PR that touches `scripts/**`, `test/**`, `package.json`, or `.github/workflows|ISSUE_TEMPLATE/**` (path-filtered so the bot's weekly data-only commits to `data/*.json` don't trigger it). Checks all scripts and lib modules with `node --check`, runs the unit tests (`npm test` → plain `node --test`, zero dependencies — the tests cover the pure helpers in `scripts/lib/`, exactly where both historical bugs (`/index` URL segment, Dynamics 365 base paths) lived), validates `package.json` parses, lints all workflows with `actionlint` (semantic workflow checks: bad `if:` expressions, wrong step outputs, invalid cron — far stronger than a YAML parse), and YAML-parses the issue templates + dependabot config with PyYAML (actionlint doesn't cover non-workflow files). This exists so a script/workflow regression (like the commit-then-never-push bug fixed shortly after this repo was created — see git history) gets caught on the PR that introduces it, instead of silently surfacing days later on the next scheduled run.
- **Workflow hardening (2026-09-04):** all three workflows pin every third-party action to a full commit SHA (with the tag in a trailing comment — Dependabot understands and updates SHA pins) and set explicit `timeout-minutes` (learn: 15, docs: 120, ci: 10) so a pathological run fails fast instead of sitting at the 6-hour default. The two monitor workflows' failure issues use a **stable title** (no run id) and a `gh`-CLI create-or-comment step: a persistent failure produces one open issue labeled `bug` (matching the human-facing "Sync Failure" template, previously mislabeled `enhancement`) with a comment per failed run, instead of a brand-new issue every week. The quarantine investigate-and-fix issue is still created fresh per occurrence via `peter-evans/create-issue-from-file` (SHA-pinned) since each one carries new content and is gated on something actually changing.
